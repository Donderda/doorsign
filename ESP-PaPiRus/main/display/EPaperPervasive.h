...
typedef enum {           // error codes
	EPD_OK, EPD_UNSUPPORTED_COG, EPD_PANEL_BROKEN, EPD_DC_FAILED
} EPD_error;
...
public:
	...
	void line(uint16_t line, const uint8_t *data, uint8_t fixed_value,
			...
			uint8_t border_byte = EPD_BORDER_BYTE_NULL, bool set_voltage_limit =
					false);

	// inline static void attachInterrupt();
	// inline static void detachInterrupt();

	EPD_Class(EPD_size size, uint16_t width, uint16_t height, int panel_on_pin,
			int border_pin, int discharge_pin, int reset_pin, int busy_pin,
			int Pin_MOSI, int Pin_MISO, int Pin_SCLK, int chip_select_pin);
