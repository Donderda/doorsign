...
EPD_Class::EPD_Class(EPD_size size, uint16_t width, uint16_t height,
		int panel_on_pin, int border_pin, int discharge_pin, int reset_pin,
		int busy_pin, int Pin_MOSI, int Pin_MISO, int Pin_SCLK,
		int chip_select_pin) :
		DisplayDriver(width, height), EPD_Pin_PANEL_ON(panel_on_pin), EPD_Pin_BORDER(
				border_pin), EPD_Pin_DISCHARGE(discharge_pin), EPD_Pin_RESET(
				reset_pin), EPD_Pin_BUSY(busy_pin), EPD_Pin_EPD_CS(
				chip_select_pin) {
	SPI_cs = chip_select_pin;
	SPI_miso = Pin_MISO;
	SPI_mosi = Pin_MOSI;
	SPI_clk = Pin_SCLK;
	this->size = size;
	...
	{
		static uint8_t cs[] = { 0x72, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0f, 0xff,
				0x00 };
	...
	case EPD_2_0: {
		...
		this->bytes_per_scan = 96 / 4;
		static uint8_t cs[] = { 0x72, 0x00, 0x00, 0x00, 0x00, 0x01, 0xff, 0xe0,
				0x00 };
		this->channel_select = cs;
		this->channel_select_length = sizeof(cs);
		this->voltage_level = 0x03;
	...
	case EPD_2_7: {
		...
		static uint8_t cs[] = { 0x72, 0x00, 0x00, 0x00, 0x7f, 0xff, 0xfe, 0x00,
				0x00 };
		this->channel_select = cs;
		this->channel_select_length = sizeof(cs);
...
void EPD_Class::setFactor(int temperature) {
	...
	static const EPD_Class::compensation_type compensation_144[3] = { { 2, 6,
			42, 4, 392, 392, 2, 6, 42 },  //  0 ... 10 Celcius
			{ 4, 2, 16, 4, 155, 155, 4, 2, 16 },  // 10 ... 40 Celcius
			{ 4, 2, 16, 4, 155, 155, 4, 2, 16 }   // 40 ... 50 Celcius
	};

	static const EPD_Class::compensation_type compensation_200[3] = { { 2, 6,
			42, 4, 392, 392, 2, 6, 42 },  //  0 ... 10 Celcius
			{ 2, 2, 48, 4, 196, 196, 2, 2, 48 },  // 10 ... 40 Celcius
			{ 4, 2, 48, 4, 196, 196, 4, 2, 48 }   // 40 ... 50 Celcius
	};

	static const EPD_Class::compensation_type compensation_270[3] = { { 2, 8,
			64, 4, 392, 392, 2, 8, 64 },  //  0 ... 10 Celcius
...
void EPD_Class::frame_fixed_timed(uint8_t fixed_value, long stage_time) {
	do {
		...
		for (uint8_t line = 0; line < this->lines_per_display; ++line) {
			this->line(this->lines_per_display - line - 1, 0, fixed_value,
					false);
		}
...
void EPD_Class::frame_data_13(const uint8_t *image, EPD_stage stage,
		bool read_progmem) {
		...
		while (block_begin < total_lines) {
			...
			for (int line = block_begin; line < block_end; ++line) {
				...
				if (full_block && (line < (block_begin + step))) {
					...
					this->line(line, &image[line * this->bytes_per_line], 0x00,
							read_progmem, stage);
				}
			}
		}
...
void EPD_Class::frame_cb_13(uint32_t address, EPD_reader *reader,
		EPD_stage stage) {
		...
		while (block_begin < total_lines) {
			...
			for (int line = block_begin; line < block_end; ++line) {
				...
				if (full_block && (line < (block_begin + step))) {
					...
					reader(buffer, address + line * this->bytes_per_line,
							this->bytes_per_line);
...
void EPD_Class::writeBuffer(uint8_t *buffer, uint8_t bitsPerPixel,
		uint16_t *palette, EPD_stage stage) {
	int repeat;
	int step;
	int block;
	if (EPD_inverse == stage) {  // stage 1
	...
	for (int i = 0; i < 40; i++) {
		for (uint16_t line = 0; line < lines_per_display; line++) {
			this->line(line, buffer + (line * dots_per_line / 8), 0, false,
					stage);
			yield();
		}
		yield();
				...
				if (full_block && (line < (block_begin + step))) {
					...
					for (int i = 0; i < this->bytes_per_line; i++) {
						...
						for (int j = 0; j < 8; j++) {
							int x = i * 8 + j;
							int y = line;
							uint8_t sourceByte = buffer[x
									+ y / 8 * this->dots_per_line];
...
void EPD_Class::nothing_frame() {
	for (uint16_t line = 0; line < this->lines_per_display; ++line) {
		this->line(line, 0, 0x00, false, EPD_normal, EPD_BORDER_BYTE_NULL,
				true);
	}
}
...
void EPD_Class::line(uint16_t line, const uint8_t *data, uint8_t fixed_value,
	...
	// border byte
	SPI_put(this->EPD_Pin_EPD_CS, border_byte);
	...
	for (uint16_t b = 0; b < this->bytes_per_line; ++b) {
		if (0 != data) {
			...
			pixels = ((pixels & 0xc0) >> 6) | ((pixels & 0x30) >> 2)
					| ((pixels & 0x0c) << 2) | ((pixels & 0x03) << 6);
			SPI_put(33, pixels);
		} else {
			SPI_put(33, fixed_value);
		}
